...
def part1():
    print(len([p for p in input if has_valid_fields(p)]))
...
def is_valid(passport):
    try:
        ...
        valid = len(byr) == 4 and (1920 <= int(byr) <= 2002)
        valid = valid and len(iyr) == 4 and (2010 <= int(iyr) <= 2020)
        valid = valid and len(eyr) == 4 and (2020 <= int(eyr) <= 2030)
        valid = valid and ((hgt.endswith('in') and (59 <= int(hgt.rstrip('in')) <= 76)) or ( hgt.endswith('cm') and (150 <= int(hgt.rstrip('cm')) <= 193)))
        valid = valid and hcl_reg.match(hcl) is not None
        valid = valid and ecl in valid_ecl
        valid = valid and pid_reg.match(pid) is not None
...
def part2():
    print(len([p for p in input if is_valid(p)]))
